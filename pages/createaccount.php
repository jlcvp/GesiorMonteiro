            ...
            if ($mail->Send()) {
				...
				<TR><TD BGCOLOR="' . $config['site']['darkborder'] . '">
				  <TABLE BORDER=0 CELLPADDING=1><TR><TD>
					...
					<BR><b><i>You will receive e-mail (<b>' . htmlspecialchars($reg_email) . '</b>) with your account details.</b></i><br>';
                ...
                $main_content .= '<br /><small>If these information do not arrive on <b>' . htmlspecialchars($reg_email) . ' in the next few minutes</b>. Please check your spam folder and add.';
            } else {
                $main_content .= '<br /><small>An error occorred while sending email! Account not created. Try again.</small>';
                $reg_account->delete();
            }
        } else {
            $main_content .= 'Your account has been created. Now you can login and create your first character. See you in ' . htmlspecialchars($config['server']['serverName']) . '!<BR><BR>';
            $main_content .= '<TABLE WIDTH=100% BORDER=0 CELLSPACING=1 CELLPADDING=4>
			<TR><TD BGCOLOR="' . $config['site']['vdarkborder'] . '" CLASS=white><B>Account Created</B></TD></TR>
			<TR><TD BGCOLOR="' . $config['site']['darkborder'] . '">
			  <TABLE BORDER=0 CELLPADDING=1><TR><TD>
